...
Discription: In this program we are taking assembly code and 
switching it into hexadeceimal on a two bit set of 16 bits
and storing it into a binary file. This then takes 
the binary file and runs it against the expect output and 
then returns a pass or fail as the result. 
...
void assemble(char* assemblyFilename, char* objectCodeFilename)
{
    ...
    while (result != 0)
    {
        ...
        if (cmp_result == 0)
        {
            ...
            char* Reg_Num1 = strtok(NULL, ",");
            char* Reg_Num2 = strtok(NULL, " ,");
            char* Reg_num3 = strtok(NULL, " \0");

            
            write_opcode(mnemonic, &machineInstruction, OPCODE_SHIFT);
            write_register_code(Reg_Num1, &machineInstruction, RT_REG_SHIFT);
            write_register_code(Reg_Num2, &machineInstruction, RM_REG_SHIFT);
            write_register_code(Reg_num3, &machineInstruction, RN_REG_SHIFT);
        ...
        if (cmp_result == 0)
        {
            ...
            write_opcode(mnemonic, &machineInstruction, OPCODE_SHIFT);
            write_register_code(reg_output, &machineInstruction, RT_REG_SHIFT);
            write_immediate(immediate, &machineInstruction, IMM_8_BIT_SHIFT);
        ...
        if (cmp_result == 0)
        {
            ...
            write_opcode(mnemonic, &machineInstruction, OPCODE_SHIFT);
            write_immediate(immediate, &machineInstruction, UCOND_BR_PC_OFFSET_BIT_SHIFT);
